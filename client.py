...
if __name__ == '__main__':
    ...
    def manage_multiple(n):
        ...
        while True:
            ...
            run_cmd_multiple(clients, n, commands)


    def run_cmd_multiple(clients, n, cmd):
        threads = []
        for i in range(n):
            print 'Thread {} running command: {} {}'.format(i, cmd[i][0], cmd[i][1])
            t = Thread(target=clients[i].run_command, args=(cmd[i][0], cmd[i][1],))
            t.start()
            threads.append(t)
        for i in range(n):
            threads[i].join()
    ...
    parser.add_argument('-m', '--multiple', help='Open given number of connections to TFTP server. Commands are entered comma seperated command list in interactive input mode. `exit` to end. Eg: >>> get one,get two', nargs=1, type=int)
